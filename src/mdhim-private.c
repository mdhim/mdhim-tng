...
struct mdhim_rm_t *_put_record(struct index_t *index, void *key, int key_len, 
	...
	if ((rl = get_range_servers(md, index, key, key_len)) == NULL) {
...
struct mdhim_bgetrm_t *_bget_records(struct mdhim_t *md, struct index_t *index,
	...
	for (i = 0; i < num_records && i < MAX_BULK_OPS; i++) {
		...
		while (rl) {
			if (rl->rank != md->mdhim_rank) {
				//Set the message in the list for this range server
				bgm = bgm_list[rl->rangesrv_num - 1];
			} else {
				//Set the local message
				bgm = lbgm;
			}
			...
			if (!bgm) {
				...
				bgm->num_records = 0;
				bgm->server_rank = rl->rank;
				bgm->mtype = MDHIM_BULK_GET;
				bgm->op = MDHIM_GET_EQ;
				bgm->index = index->id;
				bgm->index_type = index->type;
				if (rl->rank != md->mdhim_rank) {
					bgm_list[rl->rangesrv_num - 1] = bgm;
				} else {
					lbgm = bgm;
				}
			}
 ...
 */
struct mdhim_brm_t *bdel_records(struct mdhim_t *md, struct index_t *index,
				 void **keys, int *key_lens,
				 int num_records) {
	...
	for (i = 0; i < num_records && i < MAX_BULK_OPS; i++) {
		...
		if (rl->rank != md->mdhim_rank) {
			//Set the message in the list for this range server
			bdm = bdm_list[rl->rangesrv_num - 1];
		} else {
			//Set the local message
			bdm = lbdm;
		}
		...
		if (!bdm) {
			...
			bdm->num_records = 0;
			bdm->server_rank = rl->rank;
			bdm->mtype = MDHIM_BULK_DEL;
			bdm->index = index->id;
			bdm->index_type = index->type;
			if (rl->rank != md->mdhim_rank) {
				bdm_list[rl->rangesrv_num - 1] = bdm;
			} else {
				lbdm = bdm;
			}
		}
